...
#
#                                                PytorchTransformer)
#from old_module import FeedForward
...
class Generator(nn.Module):
    ...
    def __init__(self, latent_dim, vocab_size, start_token, end_token):
        ...
        # self.project = nn.Sequential(nn.Linear(latent_dim,latent_dim),nn.ReLU(),nn.Dropout(0.1),
        #                              nn.Linear(latent_dim * 2, latent_dim * 2), nn.ELU(alpha=0.1), nn.Dropout(0.1)
        #
        # )
...
class RecurrentDiscriminator(nn.Module):
    ...
    def __init__(self, hidden_size, vocab_size, start_token, bidirectional=True):
        ...
        self.rnn = nn.LSTM(hidden_size, hidden_size, num_layers=1, bidirectional=bidirectional)
        if bidirectional:
